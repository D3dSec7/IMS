...
public class signup extends JFrame {
    ...
    public signup() {
        super("Inventory System - Sign Up");
        ...
        JLabel sub = makeLabel("Sign Up to Continue", 14, false, new Color(110, 110, 110));
        ...
        card.add(field("Gmail", emailField));
        ...
        JLabel pre = new JLabel("Already have an account? ");
        pre.setFont(new Font("Segoe UI", Font.PLAIN, 12));
        JButton loginLink = linkButton("Log In", e -> {
            dispose();
            Login login = new Login();
            login.setVisible(true);
        });
        bottomLine.add(pre);
        bottomLine.add(loginLink);
    ...
    private void handleSignUp() {
        ...
        new Thread(() -> {
            boolean success = backend.registerUser(username, password, email);
            ...
            SwingUtilities.invokeLater(() -> {
                if (success) {
                    ...
                    JOptionPane.showMessageDialog(signup.this,
                            "Failed to create account. Check that username, password and email are valid and not already used.",
                            "Sign Up Failed",
                            JOptionPane.ERROR_MESSAGE);
